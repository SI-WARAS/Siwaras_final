  ...
  const SidebarContent = () => (
    <>
      ...
      <nav className="flex-1 py-4 px-4 space-y-1 overflow-y-auto">
        <p className="px-3 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2 ml-1">
          Menu
        </p>
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon
                className={`w-4 h-4 mr-3 transition-colors ${isActive ? "text-rose-600 dark:text-rose-400" : "text-slate-400 dark:text-slate-500 group-hover:text-slate-500 dark:group-hover:text-slate-400"}`}
              />
              {item.name}
            </Link>
          );
