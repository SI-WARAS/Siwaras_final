...
const Patients = () => {
  ...
  const onSubmitAdd = (data) => {
    const padukuhan =
      user?.role === "HEALTH_WORKER" ? user?.pedukuhanName : data.padukuhan;
    const address = data.detailAddress
      ? `${padukuhan}, ${data.detailAddress}`
      : padukuhan;
  ...
  const openEditModal = (patient) => {
    ...
    resetEdit({
      ...
      padukuhan:
        user?.role === "HEALTH_WORKER" ? user?.pedukuhanName : matchedPadukuhan,
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-1">
            Kelola dan pantau pasien yang terdaftar.
          </p>
        </div>
        <button
          onClick={() => {
            reset({
              padukuhan:
                user?.role === "HEALTH_WORKER" ? user?.pedukuhanName : "",
            });
      ...
      <div className="bg-white rounded-[20px] shadow-soft border border-slate-100/50 overflow-hidden flex flex-col h-[calc(100vh-12rem)] relative">
        <div className="p-5 border-b border-slate-100/50 flex flex-col sm:flex-row gap-4 justify-between bg-white shrink-0 items-center">
          ...
          <div className="relative">
            ...
            >
              <Filter
                className={`w-4 h-4 mr-2 ${showFilter ? "text-rose-500" : "text-slate-400"}`}
              />
              Urut & Filter
            </button>
            <AnimatePresence>
              {showFilter && (
                ...
                >
                  ...
                  <button
                    ...
                    className="w-full text-left px-2 py-1.5 text-[13px] hover:bg-slate-50 rounded-md text-slate-700"
                  >
                    Nama
                  </button>
                  <button
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap relative">
            <thead className="sticky top-0 z-10 bg-white">
              <tr className="text-slate-400 text-[11px] uppercase tracking-wider font-bold border-b border-slate-100/80">
                <th
                  className="px-6 py-4 cursor-pointer hover:text-slate-600"
            ...
            <tbody className="divide-y divide-slate-50/50">
              {isLoading ? (
                ...
                <tr>
                  <td
                    colSpan="5"
                    className="p-12 text-center text-slate-400 text-[13px] font-medium"
                  >
                ...
                patients.map((patient) => (
                  <tr
                    key={patient.id}
                    className="hover:bg-rose-50/30 transition-colors group bg-white cursor-pointer"
                  >
                    {/* Nama — klik langsung ke detail pasien */}
                    <td className="px-6 py-4">
                      <Link
                        to={`${basePath}/patients/${patient.id}`}
                        className="flex items-center gap-3 group/name"
                        title={`Lihat detail ${patient.name}`}
                      >
                        <div className="w-9 h-9 rounded-full bg-rose-100 flex items-center justify-center text-rose-600 font-bold text-[12px] group-hover/name:bg-rose-600 group-hover/name:text-white transition-colors shrink-0">
                          {patient.name.charAt(0)}
                        </div>
                        <div>
                          <div className="font-semibold text-slate-800 text-[13px] group-hover/name:text-rose-600 transition-colors underline-offset-2 group-hover/name:underline">
                            {patient.name}
                          </div>
                          <div className="text-[11px] text-slate-400 mt-0.5">
                            {patient.phone || "Tanpa telepon"}
                          </div>
                        </div>
                        <ChevronRight className="w-3.5 h-3.5 text-slate-300 group-hover/name:text-rose-500 transition-colors ml-1" />
                      </Link>
                    </td>
                    <td className="px-6 py-4 text-slate-500 font-mono text-[12px]">
                      {patient.nik || "-"}
                    </td>
                    ...
                    <td className="px-6 py-4 text-[13px] text-slate-500 truncate max-w-xs">
                      {patient.address}
                    </td>
                    <td className="px-6 py-4 text-right space-x-1">
                      ...
                      <button
                        ...
                        className="inline-flex items-center p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors outline-none"
                        title="Hapus pasien"
                      >
      ...
      >
        <form onSubmit={handleSubmit(onSubmitAdd)} className="space-y-5">
          <Input
            ...
            placeholder="Misal: 3402160101780001"
          />
          <Input
            label="Nama Lengkap"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Select
              label="Padukuhan"
              options={padukuhanOptions}
              {...register("padukuhan", {
                required: user?.role !== "HEALTH_WORKER",
              })}
              disabled={user?.role === "HEALTH_WORKER"}
            />
            <Input
              label="Detail Alamat"
              {...register("detailAddress")}
              placeholder="RT/RW, Jalan (Opsional)"
            />
          </div>
          <div className="flex justify-end pt-5 mt-2 border-t border-slate-100">
            ...
            <button
              ...
              className="px-5 py-2.5 text-[13px] font-medium bg-rose-600 text-white rounded-lg hover:bg-rose-700 transition-colors shadow-sm focus:ring-2 focus:ring-rose-500/20 outline-none"
            >
      ...
      >
        <form onSubmit={handleSubmitEdit(onSubmitEdit)} className="space-y-5">
          <Input
            label="NIK (Nomor Induk Kependudukan)"
            type="text"
            {...registerEdit("nik", { required: true })}
          />
          <Input
            label="Nama Lengkap"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Select
              label="Jenis Kelamin"
              options={genderOptions}
              {...registerEdit("gender", { required: true })}
            />
          </div>
      ...
      >
        <div className="space-y-5">
          <p className="text-[13px] text-slate-600 leading-relaxed">
            Apakah Anda yakin ingin menghapus pasien ini secara permanen? Semua
            rekam medis yang terkait juga akan dihapus. Tindakan ini tidak dapat
            dibatalkan.
          </p>
