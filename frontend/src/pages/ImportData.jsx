      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-1">
            Unggah ribuan data rekam medis sekaligus melalui file Excel.
          </p>
        </div>
        <button
            ...
            {status === "loading" ? (
              <div className="flex flex-col items-center">
                ...
                <p className="text-[13px] text-slate-500 mt-2 text-center max-w-sm">
                  Sistem sedang memvalidasi struktur data dan menghitung
                  kalkulasi klinis. Mohon tunggu.
                </p>
              </div>
            ) : (
              <>
                <div
                  className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-6 transition-colors ${file ? "bg-emerald-50 text-emerald-500" : "bg-rose-50 text-rose-500"}`}
                >
                  {file ? (
                ...
                {!file ? (
                  <>
                    ...
                    <p className="text-[13px] text-slate-500 text-center max-w-md mb-8">
                      Gunakan template resmi untuk mencegah kesalahan kolom.
                      Maksimal ukuran file 10MB (Hanya format .xlsx atau .xls).
                    </p>
                    <input
                      type="file"
                      ref={fileInputRef}
                  ...
                  <>
                    <h3 className="text-xl font-bold text-slate-800 mb-2 truncate max-w-xs">
                      {file.name}
                    </h3>
                    <p className="text-[13px] text-slate-500 text-center max-w-md mb-8">
                      Ukuran: {(file.size / 1024).toFixed(2)} KB. File siap
                      diproses.
                    </p>
        ...
        {status === "success" && (
          ...
          >
            ...
            <p className="text-[14px] text-slate-500 text-center max-w-md mb-8 leading-relaxed">
              Semua data telah lolos gerbang validasi dan tersimpan dengan aman
              melalui Atomic Transaction.
            </p>
            <button
              onClick={resetState}
            ...
            <div className="bg-rose-50 rounded-[24px] p-8 border border-rose-100 flex flex-col sm:flex-row items-center sm:items-start gap-6 text-center sm:text-left">
              ...
              <div className="flex-1">
                ...
                <p className="text-[14px] text-rose-600/90 leading-relaxed mb-4">
                  Prinsip <b>All-or-Nothing</b> diterapkan. Ditemukan{" "}
                  {errorReport.length} baris dengan kesalahan. Tidak ada satupun
                  data yang dimasukkan ke dalam database untuk mencegah korupsi
                  data.
                </p>
                <button
                  onClick={resetState}
                        ...
                        <td className="px-6 py-4 text-[13px] text-slate-600">
                          <ul className="list-disc pl-4 space-y-1">
                            {errItem.errors.map((msg, i) => (
                              <li key={i} className="text-rose-600 font-medium">
                                {msg}
                              </li>
                            ))}
                          </ul>
